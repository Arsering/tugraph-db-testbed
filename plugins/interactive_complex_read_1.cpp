...
/**
 * @brief Given a start Person with ID $personId, find Persons with a given first name ($firstName) that the start Person
 * is connected to (excluding start Person) by at most 3 steps via the knows relationships. Return Persons, including the
 * distance (1..3), summaries of the Persons workplaces and places of study.
 *
 */
extern "C" bool Process(lgraph_api::GraphDB &db, const std::string &request, std::string &response)
{
    ...
    for (int distance = 0; distance <= 3; distance++)
    {
        ...
        {
            person.Goto(vid);
            if (person.GetId() == start_vid || person[PERSON_FIRSTNAME].string() != first_name)
                continue;
        ...
        if (candidates.size() >= limit_results || distance == 3)
            break;
        for (auto vid : curr_frontier)
        {
            person.Goto(vid);
